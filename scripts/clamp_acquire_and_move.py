...
def main() -> int:
    ...
    try:
        ...
        advance_mm = 15.0
        R_target = pose_to_matrix(target.tolist())[:3, :3]
        z_tcp_in_base = R_target[:, 2]
        advanced_position = np.asarray(target[:3]) + z_tcp_in_base * advance_mm
        advanced_target = [round(float(v), 6) for v in advanced_position] + [round(float(v), 6) for v in target[3:]]
        code = robot.MoveL(advanced_target, 0, 0, vel=float(args.speed_mm_s),
                           acc=0.0, ovl=float(args.speed_mm_s), blendR=-1.0,
                           overSpeedStrategy=2, speedPercent=max(5, int(args.speed_mm_s)))
        if int(code) != 0:
            raise RuntimeError(f"MoveL advance along TCP Z+ rejected: {code}")
        wait_target(np.asarray(advanced_target))

        # 沿 TCP Z+ 前进到位后，再闭合夹爪到指定开度完成“夹挤”。
        # 夹到物体提前受阻（提前碰到物体）视为正常夹挤到位。
        close_env = os.environ.copy()
        close_env["PYTHONPATH"] = str(ROOT) + os.pathsep + close_env.get("PYTHONPATH", "")
        close = subprocess.run([sys.executable, str(ROOT / "scripts/set_ag95_opening.py"),
                                f"{opening_mm:.3f}", "--speed", "5", "--force", "100", "--yes"],
                               cwd=ROOT, env=close_env, text=True, capture_output=True, timeout=30, check=False)
        ...
        print(json.dumps({"center_target": target.tolist(), "advanced_target": advanced_target,
                          "width_mm": width_mm, "target_opening_mm": opening_mm,
                          "tcp_z_plus_mm": advance_mm, "status": "CLAMPED"},
                         ensure_ascii=False))
        return 0
    finally:
        try:
