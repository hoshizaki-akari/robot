...
def d435_color() -> Response:
    try:
        return Response(upstream_bytes("/api/d435/color.png"), media_type="image/png", headers={"Cache-Control": "no-store"})
    except Exception as error:
        raise HTTPException(status_code=503, detail=f"D435 彩色画面不可用：{error}") from error


@app.get("/api/d435/color.mjpg")
def d435_color_mjpeg() -> StreamingResponse:
    return upstream_mjpeg("/api/d435/color.mjpg")
...
def clamp_preview() -> Response:
    try:
        return Response(upstream_bytes("/api/platform-a/clamp/preview.png"), media_type="image/png", headers={"Cache-Control": "no-store"})
    except Exception as error:
        raise HTTPException(status_code=503, detail=f"夹挤视觉画面不可用：{error}") from error
...
@app.post("/api/robot/debug-echo")
def robot_debug_echo(ack_text: str = "") -> dict[str, Any]:
    return {"echo": ack_text}
