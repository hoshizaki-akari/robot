...
class RobotController:
    ...
    def start_clamping(self) -> None:
        if self.device.read_only:
            ...
            self.device.vision_mode = "clamp"
            try:
                self.device.start_clamp_workflow(
                    self.params.target_clamp_mm, self.params.speed_mm_s
                )
            except Exception:
    ...
    def home(self) -> None:
        ...
        if self.device.read_only:
            ...
            self.notify("夹挤复位开始：先张开夹爪，再沿安全路线返回初始位置")
            self.device.reset_clamp_workflow()
            self._set_state(ControlState.IDLE, "夹挤复位完成：夹爪全开，机械臂已回初始位置")
            return
        self._require_writable()
        self.device.reset()
