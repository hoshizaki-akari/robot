...
class CalcaneusRobotApp(tk.Tk):
    ...
    def login_dialog(self) -> bool:
        dialog = tk.Toplevel(self); dialog.title("用户登录"); dialog.geometry("360x210"); dialog.resizable(False, False); dialog.grab_set()
    ...
    def _parameter_panel(self,parent):
        ...
        items=(("撬拨位移(mm)",8.0),("夹挤位移(mm)",12.0),("速度(mm/s)",10.0),("力上限(N)",80.0),("力矩上限(Nm)",8.0),("保持时间(s)",3.0))
    ...
    def _control_panel(self,parent):
        ...
        buttons=(("开始撬拨定位",self.enter_pry_branch),("开始夹挤复位",self.start_clamp),("暂停",self.controller.pause),("继续",self.controller.resume),("安全停止",self.controller.stop),("软件回零",self.start_home),("夹爪张开 +1mm",lambda:self.controller.jog_gripper(1)),("夹爪闭合 -1mm",lambda:self.controller.jog_gripper(-1)))
    ...
    def start_pry_execution(self):
        result = self.controller.device.pry_vision.result
        if not result.get("valid"):
            raise RuntimeError(result.get("message") or "撬拨视觉结果尚未满足50–60 mm校验")
        self.log(f"撬拨启动开关已打开：水平夹持宽度 {result.get('width_mm'):.2f} mm")
        self.controller.notify("撬拨目标点已计算；运动执行接口待接入，当前未发送机器人命令")
