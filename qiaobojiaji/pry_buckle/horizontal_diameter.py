...
class HorizontalDiameterEstimator:
    ...
    def draw_overlay(image_bgr: np.ndarray, heel_mask: np.ndarray, result: dict[str, Any]) -> np.ndarray:
        ...
        left, right, center = tuple(result["contact_left_px"]), tuple(result["contact_right_px"]), tuple(result["center_px"])
        color = (0, 220, 255) if result.get("valid") else (0, 0, 255)
        cv2.line(image, left, right, color, 3); cv2.circle(image, left, 6, color, -1); cv2.circle(image, right, 6, color, -1)
        cv2.drawMarker(image, center, (255, 255, 255), cv2.MARKER_CROSS, 18, 2)
        return image
